...
#+begin_src R :tangle yes :session :results output graphics :file images/cycles_2_and_3.png :width 600
df %>%
  ...
  stat_boxplot(geom = 'errorbar', width = 0.2) +
  geom_boxplot(width = 0.2, fill = "lightgrey") +
  facet_grid(rows = vars(optm_2), cols = vars(optm_3), labeller = label_both) +
  labs(x = "Otimização de compilação aplicada",
       y = "Número de ciclos",
       title = "Número de ciclos observados conforme uso\ndas otimizações 2 e 3") +
  theme_bw() +
  theme(text = element_text(family = "Palatino", size = 16),
        axis.text.x = element_text(angle = 45, vjust = 0.6))
#+end_src
